...
<body>
    ...
    if (subjectIdStr != null && !subjectIdStr.isEmpty()) {
        ...
        try {
            ...
            String checkQuery = "SELECT COUNT(*) FROM student_subject WHERE StudentID = ? AND Subject_ID = ?";
            ...
            if (count > 0) {
                out.println("<p>You are already enrolled in this subject or your enrollment is pending approval.</p>");
            } else {
                // Insert enrollment request with 'Pending' status
                String query = "INSERT INTO student_subject (StudentID, Subject_ID, Approval_Status) VALUES (?, ?, 'Pending')";
                ...
                if (result > 0) {
                    out.println("<p>Enrollment request for subject ID " + Subject_ID + " submitted successfully. Waiting for admin approval.</p>");
                } else {
                    out.println("<p>Failed to submit enrollment request. Please try again.</p>");
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
        ...
        out.println("<p>No subject selected for enrollment.</p>");
    }
%>
